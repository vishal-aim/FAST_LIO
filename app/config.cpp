...
{
  YAML::Node root = YAML::LoadFile(path);  // throws YAML::BadFile / YAML::ParserException on failure
  ...
  // Tuning knobs the ROS launch files set via <param> rather than the yaml;
  // treated here as optional top-level keys with the same defaults.
