...
bool PacketSync::nextMeasurement(MeasureGroup &meas)
{
  ...
  if (!lidar_pushed_)
  {
    ...
    {
      lidar_end_time_ = meas.lidar_beg_time + (lidar_mean_scantime_ > 0 ? lidar_mean_scantime_ : 0.05);
    }
    else
    {
      double max_curvature = 0.0;
      for (const auto &p : meas.lidar->points)
      {
        if (p.curvature > max_curvature) max_curvature = p.curvature;
      }
      double scan_dt = max_curvature / double(1000);
      if (scan_dt < 0.5 * lidar_mean_scantime_ && lidar_mean_scantime_ > 0)
      {
        lidar_end_time_ = meas.lidar_beg_time + lidar_mean_scantime_;
      }
      else
      {
        if (scan_dt <= 0.0) scan_dt = (lidar_mean_scantime_ > 0 ? lidar_mean_scantime_ : 0.05);
        scan_num_ ++;
        lidar_end_time_ = meas.lidar_beg_time + scan_dt;
        lidar_mean_scantime_ += (scan_dt - lidar_mean_scantime_) / scan_num_;
      }
    }
